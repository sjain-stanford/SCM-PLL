...
x = 1;
for w_step_max = 0:2*pi*0.5:2*pi*20
    y = 1;
    for phi = 0:0.025:0.5
            ...
            if ( wn - (w_step_max/phi)< 10^-8 )     % special case when c1 - 2*c2 = 0 
                disp('Special case');
                wn = 2*pi*10;
            end
            ...
            while(1)
                ...
                while(1)
                    ...
                    if (isnan(corr))
                            ...
                            if res > 10^-8
                                disp('something is wrong in inbuilt solve!!');
                            end
                    else
                        wn = wn - corr;
                    end
                ...
                res = E*wn*exp(del*wn*t0) - k1*sqrt(w_step_max^2 + wn^2*phi^2 -2*w_step_max*phi*wn*del);
                if res > 10^-8
                    disp('something is wrong in newton raphson!!');
                end
