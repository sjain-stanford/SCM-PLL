...
//	PWM Clock frequency = 1MHz; hence 1 Clock Period = 10^-6 s
...
//	Lets choose the no. of clock ticks in 1 PWM period as 64 (i.e. PWM period = 64*10^-6 = 0.064 ms)
//	Thus max. output of saw-tooth wave = 64 (dec) or 40 (hex)
//	Accordingly scale the 8 bit sine table down to 6 bit (2^6 = 64) by multiplying it by (2^6)/(2^8)
...
//    	TACCR0 = 0;
// 		TACCR0 = 64;
    	TACCR1 = sin_tab[i]*64/256;
    	TACCR2 = cos_tab[i]*64/256;
    	// for a sinusoidal wave of 50Hz, the 64 points should take 20ms i.e. 20000 cycles; 20000/64 = 312.5
    	// Now exclude the excess cycles required for the *64/256 in the above two statements (say exclude 12 cycles roughly)
    	__delay_cycles(300);
    	}
