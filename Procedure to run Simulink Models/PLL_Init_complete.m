% ----------------------------------------------------------------------*
% PLL Initialization (Step 1) for PLL comparison                        *
%                                                                       *
...
fq_step = input('Frequency step observed (indicate +/-) (Hz): ');
ph_jump_max = input('\nMax. allowable phase jump (only magnitude, not sign) (rad): ');
ph_jump = input('Phase jump observed (indicate +/-) (rad): ');
...
% Without this design for worst case (opposite polarities), the error is not confined for
% all combinations. For instance, try without this line and use the
% following inputs for proposed method:

% Enter the max. permissible error band, E (rad): 0.02
% Desired settling time, t0 (s): 0.01
% 
% Max. allowable frequency step (Hz): 10
% Frequency step observed (Hz): 0
% 
% Max. allowable phase jump (rad): pi/6
% Phase jump observed (rad): pi/6
% 
% No. of cycles to be observed per change: 5
% 
% Self-consistent model optimization
% No. of iterations taken: 14
% Error band observed at t0: 0.025330
% Max error band, E: 0.020000
% Damping ratio, del: 0.912285
% Natural frequency, wn (in rad/s): 505.824410
% 
% 
% The designed values of kp, ki and Tau from Approach 2:
% 
% Proportional gain, kp = -2.837378
% Integral gain, ki = -786.605056
% Time constant, Tau = 3.607119 ms

% It is clearly seen that error 0.025330 is more than the specified band of
% E=0.02 rad.
...
while cont == 1
    ...
    switch(ch)
         case 1
            fprintf('\nWiener Method\n');
            del = 1/sqrt(2);
            wn = input('Enter the value of wn (rad/s): ');
